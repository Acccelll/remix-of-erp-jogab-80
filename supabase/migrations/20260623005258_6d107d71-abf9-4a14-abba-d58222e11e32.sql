...
BEGIN
  ...
  -- Popula o rollup. IMPORTANTE: normaliza o valor de cada fatia para que a
  -- soma das fatias por título == valor_liquido real do título. Sem isso, se
  -- a matriz importada já trouxer o valor total em cada linha de rateio
  -- (caso atual), o vencido fica multiplicado pelo número de fatias.
  WITH fatias AS (
    SELECT
      l.ref_lancamento,
      l.status_cod,
...
-- Reprocessa o rollup do snapshot ativo (com a nova fórmula).
DO $$
DECLARE v_snap UUID; v_data DATE;
BEGIN
  SELECT id, importado_em::date INTO v_snap, v_data
  FROM public.financeiro_snapshots ORDER BY importado_em DESC LIMIT 1;
  IF v_snap IS NULL THEN RETURN; END IF;

  DELETE FROM public.financeiro_evolucao_rollup WHERE snapshot_id = v_snap;

  WITH fatias AS (
    SELECT l.ref_lancamento, l.status_cod, l.status_label,
      l.valor_liquido AS titulo_valor_liquido, l.valor_baixado AS titulo_valor_baixado,
      m.cod_natureza, m.desc_natureza, m.grupo, m.cod_ccusto, m.valor_rateio,
      cct.obra_id AS fatia_obra_id
    FROM public.financeiro_lancamentos l
    JOIN public.financeiro_matriz_rateios m ON m.ref_lancamento = l.ref_lancamento
    LEFT JOIN public.centros_custo_totvs cct ON cct.codigo = m.cod_ccusto
    WHERE l.snapshot_id = v_snap AND COALESCE(l.status_cod,0) <> 18
  ),
  normalizado AS (
    SELECT f.*, SUM(f.valor_rateio) OVER (PARTITION BY f.ref_lancamento) AS soma_rateios,
      CASE WHEN f.status_cod = 3 THEN 1::NUMERIC
           WHEN COALESCE(f.titulo_valor_liquido,0)=0 THEN 0::NUMERIC
           ELSE LEAST(1::NUMERIC, GREATEST(0::NUMERIC, f.titulo_valor_baixado / NULLIF(f.titulo_valor_liquido,0)))
      END AS ratio_pago
    FROM fatias f
  ),
  calc AS (
    SELECT n.*, CASE WHEN COALESCE(n.soma_rateios,0)=0 THEN 0::NUMERIC ELSE n.valor_rateio/n.soma_rateios END AS peso_fatia
    FROM normalizado n
  )
  INSERT INTO public.financeiro_evolucao_rollup (
    snapshot_id, data_snapshot, obra_id, status_cod, status_label,
  ...
  SELECT v_snap, v_data, fatia_obra_id, status_cod, MAX(status_label),
    grupo, cod_natureza, MAX(desc_natureza),
    ROUND(SUM(titulo_valor_liquido * peso_fatia * (1 - ratio_pago))::NUMERIC, 2),
    ROUND(SUM(titulo_valor_liquido * peso_fatia * ratio_pago)::NUMERIC, 2),
    COUNT(DISTINCT ref_lancamento)
  FROM calc
  GROUP BY fatia_obra_id, status_cod, grupo, cod_natureza;
END $$;
