-- FIN-003 hardening — preserva as tabelas canônicas como somente leitura para
-- authenticated e concentra a escrita em implementação privada auditável.

CREATE OR REPLACE FUNCTION private.fn_criar_titulo_manual_impl(
...
REVOKE ALL ON FUNCTION private.fn_criar_titulo_manual_impl(
  smallint, text, text, text, date, date, text, jsonb
) FROM PUBLIC;
GRANT USAGE ON SCHEMA private TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION private.fn_criar_titulo_manual_impl(
  smallint, text, text, text, date, date, text, jsonb
...
SET search_path = ''
AS $$
  SELECT private.fn_criar_titulo_manual_impl(
    p_natureza_tipo,
    p_centro_custo,
    p_cnpj_cpf,
    p_nome,
