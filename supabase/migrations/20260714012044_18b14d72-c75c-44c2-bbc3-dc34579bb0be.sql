-- Recria tabela matriz de rateios + função fn_importar_matriz ausentes no banco
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='financeiro_matriz_rateios'
      AND policyname='fin_matriz planifik'
  ) THEN
...
GRANT EXECUTE ON FUNCTION public.fn_importar_matriz(jsonb) TO authenticated, service_role;