...
CREATE TABLE mobilizacoes_periodos (
...
CREATE INDEX idx_mob_periodos_colab ON mobilizacoes_periodos(colaborador_id);
CREATE INDEX idx_mob_periodos_datas ON mobilizacoes_periodos(data_inicio, data_fim);

ALTER TABLE mobilizacoes_periodos ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Public access mobilizacoes_periodos" ON mobilizacoes_periodos FOR ALL USING (true) WITH CHECK (true);
