...
REVOKE EXECUTE ON FUNCTION public.current_player_has_access(text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.is_current_player_gm() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.current_player_has_access(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_current_player_gm() TO authenticated, service_role;

DROP POLICY IF EXISTS "fat_nfse planifik" ON public.faturamento_nfse;
...
REVOKE ALL ON public.faturamento_nfse FROM anon;
...
BEGIN
  ...
  WITH gravadas AS (
    ...
    FROM _fat_src
    ON CONFLICT (numero_nfse, codigo_verificacao) DO UPDATE SET
