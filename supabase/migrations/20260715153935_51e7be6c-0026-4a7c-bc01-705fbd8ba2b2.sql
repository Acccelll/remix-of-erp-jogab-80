...
BEGIN
  ...
  IF p_hash_arquivo IS NOT NULL AND p_hash_arquivo <> '' THEN
    ...
    IF v_snapshot_existente IS NOT NULL THEN
      RETURN jsonb_build_object(
        ...
        'total_titulos', 0, 'total_rateios', 0, 'titulos_sem_obra', 0,
        'novos_sem_natureza', 0, 'snapshots_purgados', 0,
        'novos', 0, 'alterados', 0, 'inalterados', 0, 'total_arquivo', 0
      );
    END IF;
  END IF;
  ...
  -- Materializa rollup de Evolução de Dívidas para este snapshot.
...
-- Backfill: materializa rollup para snapshots existentes que ainda não têm linhas.
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    ...
    LEFT JOIN public.financeiro_evolucao_rollup er ON er.snapshot_id = s.id
    WHERE er.id IS NULL
    GROUP BY s.id
  LOOP
    PERFORM public.fn_materializar_financeiro_evolucao_rollup(r.id);
  END LOOP;
END $$;