...
import { Filter, X, Building2, Coffee, History } from "lucide-react";
import type { Contrato, HistoricoEntry, Obra, PeriodoContrato } from "@/types";
...
import { custoPorDia, custoPorObra, diasOcioso } from "@/lib/contratos/custoPorPeriodo";
...
interface Props {
  ...
  obras: Obra[];
  valorAtual: number;
  /** Períodos fechados vindos da rota `contratosPeriodos`. */
...
function diasEntre(iniISO: string, fimISO?: string | null): number {
  if (!iniISO) return 0;
...
const ContratoHistoricoSection: React.FC<Props> = ({
  contrato,
  obras,
  valorAtual,
  ...
  // LEAD(). Antes eram reconstruídos aqui por regex sobre uma prosa que nunca
  // casava, e todo período saía rotulado "Sem Alocação".
  const periodos = useMemo<PeriodoContrato[]>(() => {
  ...
  // ── Custo ──────────────────────────────────────────────────────────────
  // Regra e ressalva em @/lib/contratos/custoPorPeriodo: este número mudou de
  // comportamento, porque antes era calculado sobre períodos falsos.
  const inicioContrato = toISODate(contrato.inicio);
  const terminoContrato = toISODate(contrato.termino);
  const diasContrato =
    inicioContrato && terminoContrato ? Math.max(1, diasEntre(inicioContrato, terminoContrato)) : 0;
  const diaria = custoPorDia(valorAtual, diasContrato);
  const custosPorObra = useMemo(
    () => custoPorObra(periodosObraFiltrados, valorAtual, diasContrato),
    [periodosObraFiltrados, valorAtual, diasContrato],
  );
  const diasEmObra = periodosObraFiltrados.reduce((s, p) => s + (p.dias || 0), 0);
  const ocioso = diasOcioso(periodosStatusFiltrados);
  ...
  const situacao = rotuloStatus(String(contrato.status || "rascunho"));
  const vazio =
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-muted/20">
        ...
        {contrato.ocioso && (
          <Badge variant="outline" className={cn("text-xs", STATUS_TONE_CLASSES.info)}>
            Ocioso
          </Badge>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card>
          <CardHeader className="pb-1">
            <CardTitle className="text-xs text-muted-foreground">Custo por dia</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-lg font-bold">{formatBRLFromNumber(diaria)}</p>
            <p className="text-xs text-muted-foreground">
              {diasContrato > 0 ? `${diasContrato} dia(s) de vigência` : "Vigência não informada"}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1">
            <CardTitle className="text-xs text-muted-foreground">Dias alocados</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-lg font-bold">{diasEmObra}</p>
            <p className="text-xs text-muted-foreground">
              {ocioso > 0 ? `${ocioso} dia(s) ocioso` : "sem período ocioso"}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Custo por obra */}
      {custosPorObra.length > 0 && (
        <div className="rounded-lg border border-border divide-y divide-border">
          {custosPorObra.map((c) => (
            <div key={c.obraId} className="px-3 py-2 flex items-center justify-between gap-3 text-sm">
              <span className="font-medium truncate">{c.obraNome}</span>
              <span className="text-xs text-muted-foreground whitespace-nowrap">{c.dias} dia(s)</span>
              <span className="tabular-nums whitespace-nowrap">{formatBRLFromNumber(c.custo)}</span>
            </div>
          ))}
        </div>
      )}
          ...
          <div>
            ...
            <Select value={filtroTipo} onValueChange={setFiltroTipo}>
              ...
              <SelectContent>
                ...
                <SelectItem value="status">Ocioso</SelectItem>
      ...
      {periodosObraFiltrados.length > 0 && (
        <CollapsibleSection
          storageKey="historico-contrato:obras"
          title={
      ...
      {periodosStatusFiltrados.length > 0 && (
        <CollapsibleSection
          storageKey="historico-contrato:ocioso"
          defaultOpen={false}
          title={
            <span className="flex items-center gap-2">
              <Coffee className="h-4 w-4" /> Períodos ociosos
            </span>
          }
          headerExtra={
            <Badge variant="secondary" className="text-xs">
              {periodosStatusFiltrados.length} período(s) — {ocioso} dias
            </Badge>
          }
        >
      ...
      {/* Outros eventos: cadastro, mudança de situação contratual */}
      {eventosFiltrados.length > 0 && (
        <CollapsibleSection
          storageKey="historico-contrato:outros"
          defaultOpen={false}
          title={
            <span className="flex items-center gap-2">
...
export default ContratoHistoricoSection;
