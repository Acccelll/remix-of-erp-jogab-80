// Versão portfolio do card de resultado acumulado.
// Agrega todas as obras do usuário (RLS resolve o filtro).
...
export function ResultadoAcumuladoPortfolioCard() {
  const hoje = useMemo(() => new Date(), []);
  const hojeMes = format(hoje, "yyyy-MM");

  const nfsQ = useQuery({
    queryKey: ["resultado-portfolio-nfs"],
    queryFn: async () => await notasFiscaisRepo.listCurvaPortfolio(),
  });

  const bmsQ = useQuery({
    queryKey: ["resultado-portfolio-bms"],
    queryFn: async () => await bmsPrevistasRepo.listCurvaPortfolio(),
  });

  const finQ = useQuery({
    queryKey: ["resultado-portfolio-fin"],
    queryFn: async () => await financeiroRepo.vwCurvaPortfolio(),
  });
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">Resultado acumulado do portfólio</CardTitle>
        <CardDescription>Faturamento − despesas, todas as obras consolidadas</CardDescription>
      </CardHeader>
          ...
          <>
            {semDespesas && (
              <Alert>
                ...
                <AlertDescription>
                  Despesas TOTVS não disponíveis — vincule o centro de custo das obras.
                </AlertDescription>
              </Alert>
            )}
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2">
              {SERIES.map((s) => (
                <div key={s.key} className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor={`sp-${s.key}`} className="text-xs cursor-pointer">
                    {s.label}
                  </Label>
                </div>
              ))}
